...
Here's a [link to my video result](./annotated.mp4)
...
The biggest problem that I faced was to get the right points for perspective warping. It seemed to me that being a few pixels off would have been fine, because the road is very "wide" close by and quite "narrow" as we look farther towards the horizon, so merely getting the perspective at a distance to be roughly as "wide" as close up would have been a good enough transformation, but I ended up tweaking the reference points until I was finally satisfied with the result.

The second issue had to do with robustness of the detection. One problem facing the model has to do with shadows, which change the intensity of pixels. Let's say the hue and saturation is the same, but brightness is different - that can still trip up color-based thresholds. By changing the color space to HSL/HLS (or for that matter, HSV), one can isolate the lightness or value from hue and saturation. That way, if we expect the saturation to be quite high, i.e. the color is far from gray (e.g. bright yellow or white), we can ignore the effects of shadows or other lighting artifacts if we want to capture the underlying color (hue) and its intensity (saturation). This approach helped me, particularly with setting thresholds to capture the yellow left lane, while ignoring the gradients associated with the road divider (that part of the image has high absolute gradients, but the saturation and brightness aren't consistent with identifying the road).

Lastly, a bad frame or two could easily trip up the model. One way to fix this is to average over the last several frames to capture the polynomial for fitting the curve of the road. The risk here is that the more one averages, the more delayed the response is to a sharp turn, however in most cases, averaging should be innocuous, while suppressing the artifacts associated with a single bad frame, which could lead to a complete misdetection of the lane line. I found that this particularly helped during the transition from asphalt to concrete and back again.
